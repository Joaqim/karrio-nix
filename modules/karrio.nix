...
let
  ...
  useHttps =
    (cfg.extraSettings.USE_HTTPS or (if cfg.mode == "development" then "False" else "True")) == "True";
in
{
  options.services.karrio = {
    ...
    host = lib.mkOption {
      type = lib.types.str;
      default = "127.0.0.1";
      description = "Bind address for api and dashboard.";
    };
    ...
    provisionPostgresql = lib.mkOption {
      type = lib.types.bool;
      default = true;
    };
    provisionRedis = lib.mkOption {
      type = lib.types.bool;
      default = true;
    };
    ...
    database = {
      ...
      host = lib.mkOption {
        ...
        description = "Default is the postgres unix socket dir (peer auth, no password).";
      };
      port = lib.mkOption {
        type = lib.types.port;
        default = 5432;
      ...
      password = lib.mkOption {
        type = lib.types.str;
        default = "";
        description = "Empty for socket peer auth; set for TCP password auth.";
      };
    };
    ...
    enableMaildev = lib.mkOption {
      type = lib.types.bool;
      default = cfg.mode == "development";
      defaultText = lib.literalExpression ''mode == "development"'';
    };
    enableAllPlugins = lib.mkOption {
      type = lib.types.bool;
      default = cfg.mode == "development";
      defaultText = lib.literalExpression ''mode == "development"'';
    };
    ...
    lib.mkMerge [
      {
        ...
        # TODO: Expose via options as service user/group
        users.users.karrio = {
          isSystemUser = true;
          group = "karrio";
          home = "/var/lib/karrio";
        ...
        in
        {
          ...
          systemd.services.karrio-dashboard = {
            ...
            environment = {
              ...
              KARRIO_URL = if useHttps then cfg.publicApiUrl else "http://${cfg.host}:${toString cfg.apiPort}";
